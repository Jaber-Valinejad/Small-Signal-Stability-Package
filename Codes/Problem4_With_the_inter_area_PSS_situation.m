...
i=3;
A1_3=[0 1 0 0 0 0 0;
0 -D(i)/M(i) -Iq0(i)/M(i) -Id0(i)/M(i) 0 0 0;
0 0 -1/Td0(i) 0 1/Td0(i) 0 0;
0 0 0 -1/Tq0(i) 0 0 0;
0 KA(i)/TA(i)*(Kpss*(T1*T1)/(T2*T2*omegas)) 0 0 -1/TA(i) KA(i)/TA(i)*(Kpss-Kpss*(T1*T1)/(T2*T2)) KA(i)/TA(i)*(Kpss*2*T1-Kpss*(T1*T1)/(T2*T2)*(2*T2));
0 0 0 0 0 0 1;
0 1/(T2*T2*omegas) 0 0 0 -1/(T2*T2) -2/T2;];
i=4;
A1_4=[0 1 0 0 0;
0 -D(i)/M(i) -Iq0(i)/M(i) -Id0(i)/M(i) 0;
0 0 -1/Td0(i) 0 1/Td0(i);
0 0 0 -1/Tq0(i) 0;
0 0 0 0 -1/TA(i);];
AA1 = blkdiag(A1_1,A1_2,A1_3,A1_4);
AA1(15,2)=KA(3)/TA(3)*(-1)*Kpss*T1*T1/(T2*T2*omegas);
AA1(17,2)=1/(T2*T2*omegas);
for i=1:4;
B1(:,:,i)=[0 0;
...
BB1 = blkdiag(B1(:,:,1),B1(:,:,2),B_1,B1(:,:,4));
BB2 = blkdiag(B2(:,:,1),B2(:,:,2),B_2,B2(:,:,4));
EE1 = blkdiag(E1(:,:,1),E1(:,:,2),E_1,E1(:,:,4));
CC1 = blkdiag(C1(:,:,1),C1(:,:,2),C_1,C1(:,:,4));
...
C2_2=[C2(:,:,3) add0;];
%make these block diagonal matrices
CC2=blkdiag(C2(:,:,1),C2(:,:,2),C2_2,C2(:,:,4));
...
Dnew=[K1 D5;
D6 D7];
Asys=Anew-Bnn/(Dnew)*Cnn;
[V,D] = eig(Asys);
W=inv(V)';
eigenvalues=zeros(1,22);
for i=1:22
eigenvalues(i)=D(i,i);
end
...
paticipationfactor=zeros(22,22);
for i=1:22;
for j=1:22;
